...
const BASE_URL = "https://next-sale-server.vercel.app/cart";
...
const Page = () => {
  ...
  // 📋 NEW FUNCTION: Handle Email Copy
  const handleCopyEmail = async (crop) => {
    const sellerEmail = crop.ownerEmail;
    ...
    try {
      ...
      await navigator.clipboard.writeText(sellerEmail);
      toast.success(`Seller email (${sellerEmail}) copied to clipboard!`);
    } catch (err) {
      // Fallback for browsers/contexts that block the Clipboard API (e.g., non-HTTPS)
      console.error("Failed to copy text:", err);
      toast.warn(
        "Failed to use clipboard API. Please copy manually from the item details."
      );
    }
  };
            ...
            >
                ...
                {crop.image ? (
                  <img
                    ...
                    onError={(e) => {
                      ...
                      e.target.src = `https://placehold.co/128x128/9ACD32/ffffff?text=Crop`;
                    }}
                  />
                ) : (
              ...
              <div className="flex-grow w-full md:w-auto text-left">
                ...
                <div className="flex flex-wrap gap-2 text-sm font-medium">
                  ...
                  <span className="badge badge-success bg-green-100 text-green-700 p-3 rounded-full">
                    💰 totalPrice: {crop.totalPrice} TK
                  </span>
                </div>
              </div>

              <div className="mt-4 md:mt-0 md:ml-6 flex-shrink-0 flex space-x-2 w-full md:w-auto justify-end md:mt-10">
                {/* START OF NEW BUTTON */}
                <button
                  ...
                  onClick={() => handleCopyEmail(crop)}
                >
                  📋 Copy Seller Email & contact
                </button>{" "}
                {/* END OF NEW BUTTON */}
                <button
